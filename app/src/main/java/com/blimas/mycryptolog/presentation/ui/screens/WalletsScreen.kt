...
import com.blimas.mycryptolog.presentation.viewmodel.WalletUiState
...
fun WalletsScreen(
    ...
    val uiState by walletViewModel.uiState.observeAsState(WalletUiState.Loading)
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        when (val state = uiState) {
            ...
            is WalletUiState.Success -> {
                WalletsContent(
                    processedWallets = state.wallets,
...
private fun DeleteWalletDialog(onDismiss: () -> Unit, onConfirm: () -> Unit) {
    AlertDialog(
        ...
        text = { Text("Are you sure? Deleting a wallet will also delete ALL transactions associated with it.") },
        confirmButton = {
            Button(
                onClick = onConfirm,
